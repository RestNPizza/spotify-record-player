...
def run(windowed=False):
    ...
    record_image_raw = pygame.image.load(str(record_path))
    ...
    def mask_album_art(image_surface, size):
        pil_string = pygame.image.tostring(image_surface, "RGBA", False)
        pil_image = Image.frombytes("RGBA", image_surface.get_size(), pil_string)

        mask = Image.new("L", pil_image.size, 0)
        draw = ImageDraw.Draw(mask)
        draw.ellipse((0, 0, size, size), fill=255)
        draw.ellipse((size//2 - 25, size//2 - 25, size//2 + 25, size//2 + 25), fill=0)  # hole in center

        pil_image.putalpha(mask)
        pil_image.putalpha(int(255 * 0.8))  # 80% opacity

        raw_str = pil_image.tobytes()
        return pygame.image.fromstring(raw_str, image_surface.get_size(), "RGBA")
    ...
    def update_details():
        ...
        if new_details:
            ...
            try:
                ...
                masked_album_surface = mask_album_art(pygame.transform.scale(img, (500, 500)), 500)
            except Exception as e:
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                prev_h, pause_h, skip_h = prev_btn.get_height(), pause_btn.get_height(), skip_btn.get_height()
                ...
                if prev_x <= mx <= prev_x + prev_w and prev_y <= my <= prev_y + prev_h:
                    try: skip_to_previous()
                    except Exception as e: print(f"Error skipping to previous: {e}", file=sys.stderr)
                    else: threading.Thread(target=update_details, daemon=True).start()
                elif pause_x <= mx <= pause_x + pause_w and pause_y <= my <= pause_y + pause_h:
                    if is_playing:
                        try: stop_music()
                        except Exception as e: print(f"Error stopping music: {e}", file=sys.stderr)
                        else: is_playing = False; angle_speed = 0
                    else:
                        try: start_music()
                        except Exception as e: print(f"Error starting music: {e}", file=sys.stderr)
                        else: is_playing = True; angle_speed = -0.5
                elif skip_x <= mx <= skip_x + skip_w and skip_y <= my <= skip_y + skip_h:
                    try: skip_to_next()
                    except Exception as e: print(f"Error skipping to next: {e}", file=sys.stderr)
                    else:
                        record_path = random.choice(record_files)
                        record_image_raw = pygame.image.load(str(record_path))
                        record_image_raw = pygame.transform.scale(record_image_raw, (int(1080 * 1.25), int(1080 * 1.25)))
        ...
        rotated_record = pygame.transform.rotate(record_image_raw, angle)
